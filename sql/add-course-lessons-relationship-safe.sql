-- Safe version: Create course_lessons junction table and policies
-- This handles existing policies gracefully

-- 1. Create course_lessons junction table (if it doesn't exist)
...
-- 2. Add indexes for better query performance (if they don't exist)
...
-- 3. Enable RLS on course_lessons (if not already enabled)
ALTER TABLE course_lessons ENABLE ROW LEVEL SECURITY;

-- 4. Drop existing policies if they exist, then recreate them
DROP POLICY IF EXISTS "Teachers can view course_lessons for their courses" ON course_lessons;
DROP POLICY IF EXISTS "Students can view course_lessons for enrolled courses" ON course_lessons;
DROP POLICY IF EXISTS "Teachers can insert course_lessons for their courses" ON course_lessons;
DROP POLICY IF EXISTS "Teachers can update course_lessons for their courses" ON course_lessons;
DROP POLICY IF EXISTS "Teachers can delete course_lessons for their courses" ON course_lessons;

-- 5. Create RLS policies for course_lessons
...
SELECT 
    'Course_lessons table ready' as status,
